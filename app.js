...
app.get('/', async (req, res) => {
  ...
  cron.schedule('*/30 * * * * *', async () => {
    ...
    const [lastRow] = getIdealoRows.data.values.slice(-1);
    const lastRowJson = JSON.stringify(lastRow);
    const idealoLastRowJson = JSON.stringify(idealo_click_result.slice(-2)[0]);

    if (lastRowJson === idealoLastRowJson) {
      console.log('Idealo click result is up to date');
    } else {
      console.log('New data in idealo click result');
    ...
    if (
      getAwinRows_last_array_json !== awin_click_result_sum_array_last_array
    ) {
...
async function getAwinClickReport() {
  const date_to = new Date(new Date().setDate(new Date().getDate() - 0))
    .toISOString()
    .slice(0, 10);

  const date_from = new Date(new Date().setDate(new Date().getDate() - 1))
    .toISOString()
    .slice(0, 10);
  ...
  // sort by date using a stable sort algorithm
  const sorted_awin_data = data.slice().sort((a, b) => {
    const dateA = a.dateObject || (a.dateObject = new Date(a.transactionDate));
    const dateB = b.dateObject || (b.dateObject = new Date(b.transactionDate));
